...
%%%\\\noindent``urn:lsid:ipni.org:names:103307-1'',``Valid'',``'',``'',''\textbar can't construct sciName from atomic fields \textbar The scientific name and authorship are correct.  Exact Match \textbar Found name in IPNI. \textbar  Authorship: Exact Match Similarity: 1.0''.} \end{quotation}
from Table \ref{ExampleQCAssertions} is easily tranformed into the SQL query 
...
% REMOVED: make suggestions about how to deal with a disparity if one exists {IT CAN???}. 
...
%Appendix \ref{appendix:SpiderDiversity}. That Appendix describes in some detail the result of processing 
